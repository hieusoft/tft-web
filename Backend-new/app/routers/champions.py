...
def get_all(db: Session = Depends(get_db), r: redis.Redis = Depends(get_redis), _=Depends(verify_api_key)):
    cached = r.get(CHAMPIONS_CACHE_KEY)
    if cached:
        return json.loads(cached)
    champions = db.query(Champion).options(joinedload(Champion.skill)).all()
    
    result = [ChampionResponse.model_validate(c).model_dump() for c in champions]
    r.setex(CHAMPIONS_CACHE_KEY, CACHE_TTL, json.dumps(result, default=str))
    return result

@router.post("/", response_model=ChampionResponse)
def create(body: ChampionCreate, db: Session = Depends(get_db), r: redis.Redis = Depends(get_redis), _=Depends(verify_api_key)):
...
def update_champion_meta(id: int, body: ChampionUpdateMeta, db: Session = Depends(get_db), r: redis.Redis = Depends(get_redis), _=Depends(verify_api_key)):
    champ = db.query(Champion).filter(Champion.id == id).first()
    if not champ:
        raise HTTPException(status_code=404, detail="Tướng không tồn tại")
...
@router.delete("/{id}")
def delete(id: int, db: Session = Depends(get_db), r: redis.Redis = Depends(get_redis), _=Depends(verify_api_key)):
    champ = db.query(Champion).filter(Champion.id == id).first()
    if not champ:
        raise HTTPException(status_code=404, detail="Tướng không tồn tại")
